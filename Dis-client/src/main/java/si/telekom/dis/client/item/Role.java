...
public class Role extends IsSelected {
	private TextBox id;
	...
	public Role(si.telekom.dis.shared.Role rol, VerticalContainer parent)
	{
		this();
		this.item = rol;
	...
	public Role() {
		...
		lblMoveUp.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				logger.info("Gor. Parent is of class: " + Role.this.getParent().getClass().getName());
				if (Role.this.getParent().getClass().getName().contains("VerticalContainer")) {
		...
		lblDelete.addClickHandler(new ClickHandler() {
			@Override
			public void onClick(ClickEvent event) {
				logger.info("Briši. Parent is of class: " + Role.this.getParent().getClass().getName());
		...
		fp.addHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				...
				if (button == NativeEvent.BUTTON_LEFT) {
					boolean prevSelected= Role.this.getIsSelected();
					logger.info("previous selected: " + Role.this.getIsSelected());
					Role.this.setIsSelected(!prevSelected);
					logger.info("now selected: " + Role.this.getIsSelected());
				}
			}
		}, ClickEvent.getType());
		...
		fp.addHandler(new ContextMenuHandler() {
			...
			public void onContextMenu(ContextMenuEvent event) {
				...
				pop.show();
				logger.info("Click. Parent is of class: " + Role.this.getParent().getClass().getName());
				if (Role.this.getParent().getClass().getName().contains("ListBox")) {
		...
		id.addValueChangeHandler(new MyValueChangeHandler<String>(id) {
			...
			public void onValueChange(String value, String prevValue) {
				...
				{
					Role.this.item.setId(id.getText());
					Role.this.idChanged();
		...
		name.addValueChangeHandler(new ValueChangeHandler<String>() {
			...
			public void onValueChange(ValueChangeEvent<String> event) {
				// TODO Auto-generated method stub
				Role.this.item.setName(name.getText());
				Role.this.nameChanged();
			}
		});
	...
	public void setIsSelected(boolean b) {
		...
		if (parentVertCont != null) {
			for (int i = 0; i < parentVertCont.vp.getWidgetCount(); i++) {
				((Role) parentVertCont.vp.getWidget(i)).isSelected=false;
				((Role) parentVertCont.vp.getWidget(i)).vp.getElement().getStyle().setBackgroundColor("");
			}
		}
		this.isSelected = b;
		if (this.isSelected)
		...
		parentVertCont.selectionChanged();
	}
